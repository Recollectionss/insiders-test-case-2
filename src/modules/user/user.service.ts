...
export class UserService {
  ...
  async findById(userData: UserJwtDataDto): Promise<UserDto> {
    const dataValues = await this.prismaService.users.findFirst({where: {id: userData.sub}});
    return {
      id: dataValues.id,
  ...
  async findByUsername(username: string): Promise<UserDto> {
    const dataValues = await this.prismaService.users.findFirst({ where: { username } });
    return {
      id: dataValues.id,
      username: dataValues.username,
  ...
  async update(userData: UserJwtDataDto, data: UpdateUserDto): Promise<void> {
    try {
      await this.prismaService.users.update({where: {id: userData.sub}, data})
    } catch (error) {
      throw new InternalServerErrorException(error.message);
    }
  }
  ...
  private async validateByUsername(username: string): Promise<void> {
    const data = await this.prismaService.users.findFirst({where: {username}});
